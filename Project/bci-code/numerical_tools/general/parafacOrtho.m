function [varargout]=parafacOrtho(varargin);
% orthogonalise the input decomposition
%
%  [S,U{1} U{2} ...]=parafacOrtho(S,U{1},U{2},...)
%
...
actComp = abs(S)>eps*max(abs(S)); 

if ( sum(actComp)<=1 ) varargout=varargin; return; end; % nothing to do

% orthogonalise the inputs -- using a ALS algorithm
% init the bits we need
UU = zeros(sum(actComp),sum(actComp),numel(U));
for d=1:numel(U);
   V{d}=U{d};
...
[S,U{1:2}]=parafac_als_inc(A0);
[S,U{1:2}]=parafacStd(S0,U0{:}); % norm and dec order
A=parafac(S,U{:});
[S2,U2{1:2}]=parafacOrtho(S0,U0{:});
A2=parafac(S2,U2{:});
[S3,U3{1:numel(U)}]=parafacProj(A,U2{:});

A3=parafac(S3,U3);
R2=A-A2; R2(:)'*R2(:)

% with negative values
[S,U{1:2}]=parafacOrtho(S0.*[1 -1 1]',U0{:});