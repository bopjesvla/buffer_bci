...
runcorrelation  = jt_parse_cfg(cfg,'runcorrelation',true);  % whether or not to use running correlations

% Variables
nsamples = size(X,1);
nsegments = floor(nsamples/segmentlength);
margins = nan(nsegments,1);
...
if strcmpi(method,'fix')
    ...
    similarity = jt_correlation(T,X,struct('method','fwdbwd','lenseg',segmentlength,'numseg',nsegments,'runcor',runcorrelation));
