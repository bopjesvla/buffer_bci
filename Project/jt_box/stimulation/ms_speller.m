...
%       .infopositions.topy   = [float]  option to change y position of top info line
%
% OUTPUT
...
infopositions       = jt_parse_cfg(cfg,'infopositions',struct('topy',0)); yt=infopositions.topy;
viewport_topinfo    = jt_parse_cfg(cfg,'viewport_topinfo',      [0.1  0+yt   .7  .05+yt]);
viewport_cmplinfo   = jt_parse_cfg(cfg,'viewport_cmplinfo',     [.7 0+yt   1   .05+yt]);
...
if ~isfield(cfg,'function') || isempty(cfg.function)
...
if isDown
   if keyCode(kb.space) && keyCode(kb.escape)
       cleanup;
       clear Screen;  
       error('ms_speller closed remotely! [LeftShift+Escape]');
   end
end
...
%--------------------------------------------------------------------------
    function [Nmissed,vimgs] = testit(example)
        ...
        switch example
            ...
            case 34
                % Vertical Yes No with user-defined function
                symbols = {'Yes'; 'No'};
                stimuli.symbols = rand(numel(symbols),120*4)>0.5;
                cfg.function = 'jt_arrange_texels_by_position';
                cfg.params.texelsize = .33;
                cfg.params.texelpos = [.33 0 ; .33 .66];
            case 35
                % Lowlands
                symbols = {  ...
                        'A','B','C','D','E','F'; ...
                        ...
                        'S','T','U','V','W','X'; ...
                        'Y','Z','.',',','!','?'; ...
                        'space','back','@',':','enter','quit'; ...
                        };
           case 36
               info.top        = 'TOP';
               % sync only
               Tsec = 20;%30*60; % 30 min test
               stimuli.symbols = zeros(numel(symbols),Tsec*60)>0.5;
               stimuli.sync = false(1,size(stimuli.symbols,2));
               stimuli.sync(1:60:Tsec*60-1) = true;
           case 37
                % Vertical Yes No with user-defined function
                % plus all info lines
                info.top        = 'TOP';
