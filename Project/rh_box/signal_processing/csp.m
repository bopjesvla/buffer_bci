function [sf,d,Sigmai,Sigmac,SigmaAll]=csp(X,Y,dim,cent,ridge,singThresh,powThresh)
% Generate spatial filters using CSP
%
% [sf,d,Sigmai,Sigmac,SigmaAll]=csp(X,Y,[dim,cent,ridge,singThresh,powThresh]);
...
if ( nargin < 7 || isempty(powThresh) )  powThresh=1e-4; end
...
for c=1:nClass; % generate sf's for each sub-problem
   ...
   if ( 0 ) % gen eig solver method
     [W D]=eig(Sigmac(:,:,c),Sigma);D=diag(D); % generalised eigen-value formulation!
     W=real(W); % only real part is useful
   else % whiten then eigen method
     [Us,Ds] =eig(Sigma); Ds=diag(Ds); 
     si = Ds>0 & imag(Ds)<eps & Ds>max(abs(Ds))*1e-7; % numerical check
     Ws=repop(Us(:,si),'*',1./sqrt(Ds(si)+max(Ds)*1e-7)');%compute the whitener, with ridge for numerical stability
     WSigmacW=Ws'*Sigmac(:,:,c)*Ws; % apply the whitener
     [W,D]=eig(WSigmacW); D=diag(D); % decompose the whitened covariance, i.e. rotate onto CSP directions
   ...
   % eig-value for this direction in full cov - exclude low power dirs!  ... similar to regularised whitener!
   nf = sum(W.*(Sigma*W),1)'; 
   si= dc>1-singThresh | dc<0+singThresh | abs(imag(dc))>1e-8 | isnan(dc) | nf<powThresh*sum(abs(nf));
...
[sf,d,Sigmai,Sigmac]=csp(X,Y); % with data centering
