...
if ( ~isempty(opts.vals) ) % specified a set of values to keep
   if ( isstr(opts.vals) ) opts.vals={{opts.vals}}; end;
   if ( iscell(opts.vals) && isstr(opts.vals{1}) ) opts.vals={opts.vals}; end;
...
if ( ~isempty(opts.range) )
   if ( islogical(idx{dim}) ) idx{dim}=find(idx{dim}); end;
      ...
      switch lower(opts.range);
       case 'before';                if(nidx~=1) error('1 idx for before');end
        idx{dim} = 1:idx{dim}(1)-1;
       case 'after';                 if(nidx~=1) error('1 idx for after'); end
        idx{dim} = idx{dim}(1)+1:szX(dim);
       case 'between';        if(mod(nidx,2)~=0) error('2 idx for between');end
        ind=false(szX(dim),1);
        for i=1:2:nidx-1; ind(idx{dim}(i):idx{dim}(i+1))=true; end;
        idx{dim} = ind; %idx{dim}(1):idx{dim}(min(end,2));
       case 'outside';        if(nidx>2||nidx<1) error('2 idx for outside');end
        ind=true(szX(dim),1);
        for i=1:2:nidx-1; ind(idx{dim}(i):idx{dim}(i+1))=false; end;
